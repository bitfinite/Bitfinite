...
unsigned int nStakeMinAge = 24 * nOneHour; // 24 hr as zero time weight
unsigned int nStakeMaxAge = 60 * nOneDay; // 60 days as full weight
unsigned int nStakeTargetSpacing = 1.5 * 60; // 1.5-minute stakes spacing
int64_t nTargetSpacing = 1.5 * 60;  // Same as the above
unsigned int nModifierInterval = 1 * nOneHour; // time to elapse before new modifier is computed

int nCoinbaseMaturity = 23;
...
const string strMessageMagic = "Bitfinite Signed Message:\n";
...
void ThreadScriptCheck(void*) {
	...
	RenameThread("Bitfinite-scriptch");
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
	...
	bool fEnforceBIP30 = true; // Always active in Bitfinite
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
	...
	if (fProofOfStake)
	{
		...
		// Bitfinite: check proof-of-stake block signature
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
	...
	if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
	{
		...
		printf("*** %s\n", strMessage.c_str());
		uiInterface.ThreadSafeMessageBox(strMessage, "Bitfinite", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
		StartShutdown();
		return false;
	}
	return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
	if (fTestNet)
	{
		...
		nCoinbaseMaturity = 23; // test maturity is 10 blocks
	...
	if (mapBlockIndex.empty())
	{
		...
		const char* pszTimestamp = "Bitfinite Genesis Rising Today";
		CTransaction txNew;
		txNew.nTime = 1510111800;
		txNew.vin.resize(1);
		txNew.vout.resize(1);
		...
		block.nVersion = 1;
		block.nTime = 1510111800;
		block.nBits = bnProofOfWorkLimit.GetCompact();
		block.nNonce = 1240724; // !fTestNet ? 1575379 : 46534;

		block.print();
		//// debug print
		assert(block.hashMerkleRoot == uint256("0xbee05ba9af0e7024dc37eae681750a6a8572ed9a0d75b58fc735b6313d04bacd"));
...
unsigned char pchMessageStart[4] = { 0x2f, 0x37, 0xe5, 0x4d };
