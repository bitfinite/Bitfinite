...
void ThreadIRCSeed(void* parg)
{
    ...
    RenameThread("Bitfinite-ircseed");
...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #BitfiniteTEST2\r");
            Send(hSocket, "WHO #BitfiniteTEST2\r");
        } else {
            // randomly join #Bitfinite00-#Bitfinite05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #Bitfinite%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #Bitfinite%02d\r", channel_number).c_str());
        }
