Bitfinite-qt: Qt4 GUI for Bitfinite
===============================
...
Alternatively, install Qt Creator and open the `Bitfinite-qt.pro` file.

An executable named `Bitfinite-qt` will be built.
...
A warning for people using the *static binary* version of Bitfinite on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of Bitfinite is linked against libdb5.3.
...
installed causes Bitfinite-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
